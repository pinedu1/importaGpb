...
class Logradouro implements Command {
	...
	private String montaNomeComplemento( String nome, String tipoLogradouro, String lote, String nomeComplemento, String numeroComplemento ) {
		String a = "${tipoLogradouro} ${nome} ${lote} ${nomeComplemento} ${numeroComplemento}"
	...
	private void leLogradouro(String path, String uf) throws Exception {
		...
		if ( inserts ) {
			db.withBatch(500, "INSERT INTO logradouro(id,  version,  abrvact,  bairro_fim_id, bairro_ini_id,   cep, chavedne,  cidade_id,   complemento, distrito_id,   final_trecho,  grande_usuario, inicio_trecho,   nome, nome_complemento,   nome_lote,  numero_complemento, paridade,   preposicao,  sistema, status_logradouro,  tipo_log_id, titulo, nome_normalizado) VALUES (nextval('seq_logradouro'), :version, :abrvact, :bairro_fim_id, :bairro_ini_id, :cep, :chavedne, :cidade_id, :complemento, :distrito_id, :final_trecho, :grande_usuario, :inicio_trecho, :nome, :nome_complemento, :nome_lote, :numero_complemento, :paridade, :preposicao, :sistema, :status_logradouro, :tipo_log_id, :titulo, :nome_normalizado);") { BatchingPreparedStatementWrapper bpsw ->
				inserts.each {
					bpsw.addBatch(it)
				}
			}
...
CREATE TABLE IF NOT EXISTS logradouro (
    ...
    CONSTRAINT "FKsqhqblp4bm9mdo0ck39seqjci" FOREIGN KEY (cidade_id)
		...
		""")
		db.execute("""
CREATE INDEX IF NOT EXISTS "idxLogAtivo" ON logradouro USING btree (ativo);
CREATE INDEX IF NOT EXISTS "idxLogBaiFim" ON logradouro USING btree (bairro_fim_id);
CREATE INDEX IF NOT EXISTS "idxLogBaiIni" ON logradouro USING btree (bairro_ini_id);
CREATE INDEX IF NOT EXISTS "idxLogCep" ON logradouro USING btree (cep);
CREATE INDEX IF NOT EXISTS "idxLogChaveDNE" ON logradouro USING btree (chavedne);
CREATE INDEX IF NOT EXISTS "idxLogCid" ON logradouro USING btree (cidade_id);
CREATE INDEX IF NOT EXISTS "idxLogNome" ON logradouro USING btree (nome);
CREATE INDEX IF NOT EXISTS "idxLogNomeNormal" ON logradouro USING btree (nome_normalizado);
CREATE INDEX IF NOT EXISTS "idxLogPai" ON logradouro USING btree (logradouro_pai_id);
CREATE INDEX IF NOT EXISTS "idxLogTipoLog" ON logradouro USING btree (tipo_log_id);
""")
